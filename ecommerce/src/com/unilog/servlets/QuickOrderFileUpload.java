...
/**
 * Servlet implementation class QuickOrderFileUpload
 */
public class QuickOrderFileUpload extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    public QuickOrderFileUpload() {
        super();
    }
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int uploadItemCnt = CommonUtility.validateNumber(CommonDBQuery.getSystemParamtersList().get("QUICK_ORDER_RECORD_LIMIT"));
		if(uploadItemCnt<1){
			uploadItemCnt = 400;
		}
		...
		JsonObject innerObjectArray =null;
		String[] headingArray = null;
		JsonArray jasonArrayObject = new JsonArray();
		...
		try{
			while(files.hasMoreElements()) {
				...
				if(copyFile(fileToUpload, new File(saveFile), session) == true && fileValidate){
					try{
						...
						if(lastRowNum > 0 && lastRowNum<=uploadItemCnt){
							...
							for (int rowNum = 0; rowNum <=lastRowNum; rowNum++){
								...
								innerObjectArray = new JsonObject();
								...
								for(int celNum=0; celNum<headerRow.getLastCellNum(); celNum++){
										...
										if(rowNum==0){
											...
											if(headingArray!=null && headingArray.length>celNum && headingArray[celNum]!=null && !headingArray[celNum].trim().equalsIgnoreCase("")){
												if(headingArray[celNum].trim().equalsIgnoreCase("Description")){
													...
													innerObjectArray.addProperty("keyword", xlValue);// --- Changes due to new Quick Order pattern refer PSS Pattern 
													isKeywordExist = true;
													...
													innerObject.addProperty(headingArray[celNum], xlValue);
													innerObjectArray.addProperty("qty", xlValue);// --- Changes due to new Quick Order pattern refer PSS Pattern 
												} else{
									...
									if(rowNum!=0){
										...
										//jsonObject.add("Items"+rowNum, innerObject);// --- Changes due to new Quick Order pattern refer PSS Pattern 
										jsonObject.add("Items"+rowNum, innerObjectArray);
							...
							invalidFile = true;
							if(lastRowNum<1){
								invalidFileError = LayoutLoader.getMessageProperties().get(locale).getProperty("quickOrder.label.fileupload.empty");								
							}else if(lastRowNum>uploadItemCnt){
								recordLimitCrossed = true;
								invalidFileError = String.format(LayoutLoader.getMessageProperties().get(locale).getProperty("quickOrder.label.fileupload.overflow"), uploadItemCnt, uploadItemCnt, uploadItemCnt);
							}
						}
				...
				if(recordLimitCrossed){
					invalidFile = true;
				}else if(!isKeywordExist){
					invalidFileError = LayoutLoader.getMessageProperties().get(locale).getProperty("quickOrder.label.fileupload.invalid.format");
					invalidFile = true;
				}

				if(invalidFile){
					sendErrorResponse(response, invalidFileError);
					System.out.println(fileName +": Invalid format file Please upload .xlsx file.");
				}else if(jsonObject!=null && jsonObject.entrySet()!=null && jsonObject.entrySet().size()>0){
					for (Entry<String, JsonElement> mapOject: jsonObject.entrySet()) {
						mapOject.getKey();
						mapOject.getValue();
						jasonArrayObject.add(mapOject.getValue());
					}
					System.out.println(jasonArrayObject.toString());
					//out.print(jasonArrayObject.toString());
					sendResponseJsonArray(response, jasonArrayObject.toString(),jasonArrayObject);
				}
			/*if(jsonObject!=null){
	...
	private void sendResponseJsonArray(HttpServletResponse response, String excelPartListJson,JsonArray jasonArrayObject) {
		PrintWriter out= null;
		try {
			out = response.getWriter();
		} catch (IOException e) {
		...
		if(jasonArrayObject!=null){
			out.print(jasonArrayObject);
		}
