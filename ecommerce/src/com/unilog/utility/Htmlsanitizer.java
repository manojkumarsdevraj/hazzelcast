...
public class Htmlsanitizer {
	...
	private static final Pattern COLOR_NAME = Pattern.compile("(?:aqua|black|blue|fuchsia|gray|grey|green|lime|maroon|navy|olive|purple|red|silver|teal|white|yellow)");// The 16 colors defined by the HTML Spec (also used by the CSS Spec)
	private static final Pattern COLOR_CODE = Pattern.compile("(?:#(?:[0-9a-fA-F]{3}(?:[0-9a-fA-F]{3})?))");// HTML/CSS Spec allows 3 or 6 digit hex to specify color
	...
	private static final Pattern HTML_TITLE = Pattern.compile("[\\p{L}\\p{N}\\s\\-_',:\\[\\]!\\./\\\\\\(\\)&]*");// force non-empty with a '+' at the end instead of '*'
	...
	/*public String htmlSanitize(String value){
		try {
			value = POLICY_DEFINITION.sanitize(value);
		} catch (Exception e) {
			e.printStackTrace();
		}
		 return value;
