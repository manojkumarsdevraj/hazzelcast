...
import com.unilog.products.serviceimpl.ProductHunterImpl;
...
public class ProductHunterSolrV2 {
	
	
	//local server
	//static String solrURL = "http://192.168.1.189/solrVanMeter/";
 public static String solrURL = CommonDBQuery.getSystemParamtersList().get("SOLR_URL");
	...
	try
	{
		...
		if(keyWord!=null && !keyWord.trim().equalsIgnoreCase(""))
		{
			 ...
			 if(validateForSearch!=null && validateForSearch.length>1)
			 {
				...
				multiKeyword = ProductHunterUtility.getInstance().escapeQueryCulpritsWithoutWhiteSpace(keyWord) + multiKeyword;
				multiKeyword = ProductHunterUtility.getInstance().escapeQueryCulpritsWithoutWhiteSpace(keyWord);
		...
		if(treeId >0)
		 {
			...
			if(CommonDBQuery.getSystemParamtersList().get("PRODUCT_ATTRIBUTE")!=null && CommonDBQuery.getSystemParamtersList().get("PRODUCT_ATTRIBUTE").trim().equalsIgnoreCase("Y")){
				faucetField = ProductHunterSolrUltimateV2.getProductSelectableAttribute(productConfigUrl, "", facetQueryString, facetInnderUrl, productId,refinedList,true,true,true,isDifferentiator,true,new ArrayList<String>(),new ArrayList<String>());
			}
			
			if(faucetField==null){
				attributePrepend = "attr_";
				faucetField = ProductHunterSolrUltimateV2.getFaucetFieldByName(facetConUrl, "", facetQueryString, facetInnderUrl, treeId,refinedList,true,true,isDifferentiator,true,new ArrayList<String>(),new ArrayList<String>());
			}
		 }
	...
	try
	{
		...
		if(attrFilterList!=null && !attrFilterList.trim().equalsIgnoreCase(""))
		{
			...
			for(String sArr:tempArr)
			{
				...
				if(appendFilter!=null && appendFilter.length>0)
				{
					...
					for(String append:appendFilter)
					{
						...
						if(replaceFilterVal!=null)
						{
							...
							{
								replaceFilterVal = ProductHunterUtility.getInstance().escapeQueryCulpritsWithoutWhiteSpace(replaceFilterVal);
				...
				rebuildAttrNav = rebuildAttrNav+c+ProductHunterUtility.getInstance().escapeQueryCulprits(attrArray[0])+":("+buildFilterKey+")";
				c = "~";
			}
		...
		if(treeId >0)
		 {
			...
			if(faucetField==null){
				attributePrepend = "attr_";
				faucetField = ProductHunterSolrUltimateV2.getFaucetFieldByName(facetConUrl, "", facetQueryString, facetInnderUrl, treeId,refinedList,true,true,isDifferentiator,true,finalRemoveList,new ArrayList<String>());
			}
		 }
		 ...
		 if(faucetField!=null && faucetField.length>0)
		 {
			 ...
			 String fl = "itemid,partnumber,altPartNumber1,brand,manufacturerName,manfId,manfpartnumber,brandID,salesUom,qtyAvailable,packageQty,productItemCount,imageName,imageType,attr_*";
				...
				{
					multiFaucetResult = ProductHunterSolrUltimateV2.generateFaucetFilter("productId:"+productId, attrFtrGlobal, fq, false, false,null,null,filteredMultList);
				}
				
				
				ArrayList<ProductsModel> attrFilteredList = ProductHunterSolrUltimateV2.buildFacetFilter(facetFeild, multiFaucetResult, filteredMultList);
	...
	try{
		...
		if(resultCount>0)
		{
			...
			faucetField  = CommonDBQuery.getDefaultFacet().toArray(faucetField);
			 ...
			 ProductsModel searchResultData = ProductHunterSolrUltimateV2.solrNavigationSearchResult("*:*", solrURL+"/mainitemdata", fqNav.split("~"), 0, 1, "", faucetField, "", "partnumber", SolrQuery.ORDER.asc, true, "catSearchId:"+treeId,null,fq,null,null,false);
		...
		if(resultCount>0)
		{
			...
			faucetField  = CommonDBQuery.getDefaultFacet().toArray(faucetField);
			 ...
			 if(treeId==0){
				...
				{
					ProductsModel searchResultData = ProductHunterSolrUltimateV2.solrNavigationSearchResult("*:*", solrURL+"/mainitemdata", fqNav.split("~"), fromRow, toRow, "", faucetField, "", sortField, sortOrder, true, "catSearchId:"+treeId,null,fq,null,null,false);
					ArrayList<ProductsModel> itemLevelFilterData = searchResultData.getItemDataList();
...
public static ArrayList<ProductsModel> breadCrumbList(int subsetId,int generalSubset,int treeId)
{
	...
	try
	{
		...
		req.setMethod(METHOD.POST);
		SolrQuery query = new SolrQuery();
		String fq="type:"+indexType+"~"+"categoryID:"+treeId;
		int resultCount = 0;
		query.setQuery("*:*");
		//String fq = "{!join from=itemid to=id fromIndex=itempricedata}type:"+indexType;
		String attrFtr[] = fq.split("~");
...
public static ProductsModel searchCategory(String keyWord,int subsetId,int generalSubset)
{
	...
	try
	{
		 ...
		 if(generalSubset>0 && generalSubset!=subsetId)
			 indexType = "PH_SEARCH_"+generalSubset+"_"+subsetId;
		String facetConUrl = solrURL+"/catalogdata";
		 String facetQueryString = "categorysearch:\""+ProductHunterUtility.getInstance().escapeQueryCulpritsWithoutWhiteSpace(keyWord.toLowerCase())+"\"";
		 
		 searchCatResult =  ProductHunterSolrUltimateV2.checkCategoryExist(facetConUrl, facetQueryString,indexType);
...
public static ArrayList<ProductsModel> SearchStaticContent(String keyWord,String searchType,int fromRow,int noOfPage)
{
	ArrayList<ProductsModel> itemLevelFilterData = new ArrayList<ProductsModel>();
	 try
	 {
		 if(keyWord!=null && !keyWord.trim().equalsIgnoreCase(""))
		 {
			 keyWord = ProductHunterUtility.getInstance().escapeQueryCulpritsWithoutWhiteSpace(keyWord);
			 ...
			 {
				 itemLevelFilterData = ProductHunterSolrUltimateV2.staticContentSearch(connUrl, queryString, keyWord,fromRow,noOfPage);
			 }
...
public static HashMap<String, ArrayList<ProductsModel>> searchNavigation(ProductHunterModel pmodel,String keyWord,int subsetId,int generalSubset,int fromRow,int toRow,String requestType,String taxonomyId, int treeId,String attrFilterList,String brandId, String sortBy, int resultPerPage, String narrowKeyword, int buyingCompanyId,int userId, boolean isCategoryNav,boolean exactSearch,String viewFrequentlyPurcahsedOnly)
{
	HashMap<String, ArrayList<ProductsModel>> searchResultList = new HashMap<String, ArrayList<ProductsModel>>();
	try{
		StringBuilder  fq = new StringBuilder();
		fq.append("subsetIds:").append(subsetId);
		 if(generalSubset>0 && generalSubset!=subsetId){
			 fq = new StringBuilder();
			 fq.append("subsetIds:(").append(subsetId).append(" OR ").append(generalSubset).append(")");
		 }
		 fq = fq.append("~taxonomyId:").append(taxonomyId);
		 if(pmodel==null){
			 pmodel = new ProductHunterModel();
		 }
		 pmodel.setFq(fq.toString());
		 pmodel.setKeyWord(keyWord);
		 pmodel.setSubsetId(subsetId);
		 pmodel.setGeneralSubset(generalSubset);
		 pmodel.setFromRow(fromRow);
		 pmodel.setToRow(toRow);
		 pmodel.setRequestType(requestType);
		 pmodel.setTaxonomyId(taxonomyId);
		 pmodel.setTreeId(treeId);
		 pmodel.setAttrFilterList(attrFilterList);
		 pmodel.setBrandId(brandId);
		 pmodel.setSortBy(sortBy);
		 pmodel.setResultPerPage(resultPerPage);
		 pmodel.setNarrowKeyword(narrowKeyword);
		 pmodel.setBuyingCompanyId(buyingCompanyId);
		 pmodel.setUserId(userId);
		 pmodel.setExactSearch(exactSearch);
		 pmodel.setCategoryNav(isCategoryNav);
		 pmodel.setViewFrequentlyPurcahsedOnly(viewFrequentlyPurcahsedOnly);
		 ProductHunter ps = new ProductHunterImpl();
		 searchResultList = ps.solrSearchNavigation(pmodel);
	}catch (Exception e) {
		e.printStackTrace();
...
public static LinkedHashMap<String, ArrayList<ProductsModel>> getPromotionDeals(String promotionType)
{
	...
	try
	{
		ArrayList<String> phList = ProductHunterSolrUltimateV2.phTypeList();
		for(String pList:phList)
		{
			ArrayList<ProductsModel> promotionList = ProductHunterSolrUltimateV2.productPromotion(pList,promotionType);
...
public static String  buildKeyword(String keyWord)
{
	...
	String pattern = "[^A-Za-z0-9]";
	try{
		singleKeyword = ProductHunterUtility.getInstance().escapeQueryCulpritsWithoutWhiteSpace(keyWord);
...
static ArrayList<ProductsModel> advanceMpnSearch(String queryString, int start, int resultPerPage, String narrowKeyWord, String fqNav, String sortField, ORDER sortOrder,boolean exactSearch, String manfId ) {
	...
	try {
		...
		int resultCount = 0;
			...
			if(narrowKeyWord != null && narrowKeyWord.trim().length() > 0){
				narrowKeyWord = ProductHunterUtility.getInstance().escapeQueryCulprits(narrowKeyWord.trim());
...
static ArrayList<ProductsModel> advanceUPCSearch(String queryString, int start, int resultPerPage, String narrowKeyWord,String fqNav, String sortField, ORDER sortOrder,boolean exactSearch) {
	String keyWord = buildKeyword(queryString);
	if(exactSearch) {
		keyWord = buildExactKeyword(queryString);
	}
 ...
 * @return
 */
static ArrayList<ProductsModel> advancePartNumberSearch(String queryString, int start, int resultPerPage, String narrowKeyWord, String fqNav, String sortField, ORDER sortOrder,boolean exactSearch) {
	String keyWord = buildKeyword(queryString);
	if(exactSearch) {
		keyWord = buildExactKeyword(queryString);
	}
	...
	try {
		...
		int resultCount = 0;
			...
			if(resultCount < 1 && !exactSearch) {
				...
				if(narrowKeyWord != null && narrowKeyWord.trim().length() > 0){
					narrowKeyWord = ProductHunterUtility.getInstance().escapeQueryCulprits(narrowKeyWord);
					query.setFilterQueries(new String[]{fqNav, narrowKeyWord, "defaultCategory:Y"});
				} else {
					query.setFilterQueries(new String[]{fqNav, "defaultCategory:Y"});
				}
...
public static ArrayList<ProductsModel>  customerPartNumberSearch(String queryString, int start, int resultPerPage,  int subsetId, int buyingCompanyId){
	String keyWord = buildKeyword(queryString);
	keyWord = ProductHunterUtility.getInstance().escapeQueryCulprits(queryString);
...
public static ArrayList<ProductsModel> myProductGroupSearch(String keyWord,int savedGroupId,int userId,int subsetId,int generalSubsetId,int fromRow,int resultPerPage,String sortBy, String eclipseSessionId, String entityId, String homeTeritory, String type,String userName)
{
	...
	try
	{
		...
		String faucetField[] = null;
		 ...
		 keyWord = ProductHunterUtility.getInstance().escapeQueryCulprits(keyWord);
		 keyWordWithOutSpace = ProductHunterUtility.getInstance().escapeQueryCulpritsWithoutWhiteSpace(keyWordWithOutSpace);
		 ...
		 */
		 ...
		 if(includePartNumberSearch && customerPartnumberSearch)
		 {
			...
			/* if(narrowKeyword!=null && !narrowKeyword.trim().equalsIgnoreCase(""))
			 ...
			 searchResultData = ProductHunterSolrUltimateV2.solrSearchResultCustomerPartNumber( narrowSearchKey, queryString, connUrl, attrFtr, attrFtr1, fromRow, resultPerPage,connUrlSub,faucetField);
		 ...
		 if(itemLevelFilterData.size()<1)
		 {
			...
			if(includePartNumberSearch)
			 ...
			 connUrlSub = solrURL+"/mainitemdata";
			 searchResultData = new ProductsModel();
			 searchResultData = ProductHunterSolrUltimateV2.solrSearchResult(narrowSearchKey, queryString, connUrl, attrFtr, fromRow, resultPerPage,connUrlSub,faucetField,origKeyWord,sortField,sortOrder,includePartNumberSearch,false,itemLevelFilterData,idList,null,fq,null,false,false,false,true,"", isPromotion,  isExcludeItems,  elevateIds,  excludeItems, bannerEntity);		
			 itemLevelFilterData = searchResultData.getItemDataList();
		 ...
		 if(itemLevelFilterData.size()<1)
		 {
			...
			if(!includePartNumberSearch)
			{
				queryString = multiKeywordWildCard;
				 ...
				 connUrlSub = solrURL+"/mainitemdata";
				 searchResultData = new ProductsModel();
				 searchResultData = ProductHunterSolrUltimateV2.solrSearchResult(narrowSearchKey, queryString, connUrl, attrFtr, fromRow, resultPerPage,connUrlSub,faucetField,origKeyWord,sortField,sortOrder,includePartNumberSearch,false,itemLevelFilterData,idList,null,fq,null,true,false,false,true,"", isPromotion,  isExcludeItems,  elevateIds,  excludeItems, bannerEntity);
				 itemLevelFilterData = searchResultData.getItemDataList();
				/* if(suggestedValue==null)
					 suggestedValue = searchResultData.getSuggestedValue();System.out.println("Suggtion Set : " + suggestedValue +" - " + searchResultData.getSuggestedValue());
*/
...
public static LinkedHashMap<String, ArrayList<MenuAndBannersModal>> generateBrandMenu(int rows)
{
	...
	try
	{
		...
		query.setStart(0);
		query.setRows(rows);
		query.addSortField("displaylabel", SolrQuery.ORDER.asc );
		query.setFilterQueries(attrFtr);
...
public static ArrayList<ProductsModel> getItemDetailsForGivenPartNumbers (int subsetId,int generalSubsetId,String partNumberList,int brandID,String searchBrand,String searchBySolrFieldName)
{
	...
	public static ArrayList<MenuAndBannersModal> getBannerListByName(String bannerListName){
			...
			try{
				...
				while (itr.hasNext()) {
					...
					if(CommonUtility.validateString(imgList).length()>0){
						 ...
						 itemDetailObject = new ArrayList<ProductsModel>(ProductHunterSolrUltimateV2.getImageList(imgList));
		        	...
		        	if(CommonUtility.validateString(docList).length()>0){
			         itemDetailObject = new ArrayList<ProductsModel>(ProductHunterSolrUltimateV2.getDoucmentList(docList));
		        	...
		        	if(CommonUtility.validateString(videoList).length()>0){
			         itemDetailObject = new ArrayList<ProductsModel>(ProductHunterSolrUltimateV2.getVideoList(videoList));
		        	}
		        	itemDetailList.put("videoLinks", itemDetailObject);
		        	
		       	 	itemDetailObject = new ArrayList<ProductsModel>();
		        	if(CommonUtility.validateString(linkedItems).length()>0){
		        	 itemDetailObject = ProductHunterSolrUltimateV2.getLinkedItemList(linkedItems, subsetId, generalSubset);
		        	}
		...
		public static ArrayList<ProductsModel> getFeatureProduct(int subsetId, int generalSubset, int fromRow, int rowSize) {
			ArrayList<ProductsModel> itemLevelFilterData = new ArrayList<ProductsModel>();
			HttpSolrServer server = null;
			HttpSolrServer server1 = null;
			String idList = "";
							...
							try{
								...
								if(CommonDBQuery.getSystemParamtersList().get("GET_ITEM_LEVEL_BREADCRUMB")!=null && CommonDBQuery.getSystemParamtersList().get("GET_ITEM_LEVEL_BREADCRUMB").trim().equalsIgnoreCase("Y")){
									if(doc.getFieldValue("catSearchId")!=null && doc.getFieldValue("categoryNamePath")!=null){
										itemModel.setItemBreadCrumb(ProductHunterSolrUltimateV2.getItemBreadCrumb((List<Integer>) doc.getFieldValue("catSearchId"),(String) doc.getFieldValue("categoryNamePath")));
									}
								}
								if(CommonDBQuery.getSystemParamtersList().get("SEARCH_BY_PRODUCT")!=null && CommonDBQuery.getSystemParamtersList().get("SEARCH_BY_PRODUCT").trim().equalsIgnoreCase("Y")){
									if(doc.getFieldValue("productItemCount")!=null){
		...
		public static ProductsModel findItemByPartNumber(String partNumber,int userSubsetId, int generalSubsetId) {
			String filterType = "PH_SEARCH_" + userSubsetId;
			 if(generalSubsetId > 0) {
				 filterType += "_" + generalSubsetId;
			 }
			ProductsModel product = null;
			HttpSolrServer solrServer = ConnectionManager.getSolrClientConnection(CommonDBQuery.getSystemParamtersList().get("SOLR_URL")+"/itemdetaildata");
			solrServer.setParser(new XMLResponseParser());
			SolrQuery query = new SolrQuery("partnumber:"+ partNumber);
			//query.setRequestHandler("/mainitem_keywordsearch");
			...
			try {
				...
				if(results.getNumFound() == 1) {
					product = extractProduct(results.iterator().next());
					if(product != null) {
						String itemPriceId = getItemPriceIdByItemId(product.getItemId(), userSubsetId, generalSubsetId);
						if(itemPriceId == null || itemPriceId.trim().length() <= 0) {
							product = null;
						}
					}
				}
		...
		private static ProductsModel extractProduct(SolrDocument solrDoc) {
			...
			product.setMinOrderQty(Integer.parseInt(solrDoc.getFieldValue("minordqty").toString()));
			product.setOrderInterval(Integer.parseInt(solrDoc.getFieldValue("orderQtyInterval").toString()));
			product.setShortDesc(solrDoc.getFieldValue("description").toString());
			product.setUpc(solrDoc.getFieldValue("upc").toString());
			product.setImageName(solrDoc.getFieldValue("imageName").toString());
			product.setImageType(solrDoc.getFieldValue("imageType").toString());
