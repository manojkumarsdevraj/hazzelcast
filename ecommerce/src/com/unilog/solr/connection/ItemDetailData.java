...
public class ItemDetailData {
	...
	private ItemDetailData() {
		try {
		    server = new HttpSolrServer(CommonUtility.validateString( CommonDBQuery.getSystemParamtersList().get("SOLR_URL")+"/itemdetaildata"));
	...
	public static ItemDetailData getInstance() {
        if (instance == null){
            synchronized(ItemDetailData.class){
               if(instance == null){
